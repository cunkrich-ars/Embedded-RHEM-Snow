...
C     t(26000)   real          interpolated time (sec),

C     z(26000)   real          interpolated depth (m or ft),
...
C------------------------------------------------------------------------------
      ...
      integer m, n, i, j, k, ierr, filen, kr, krtemp, kk, kkp1,
     &        nrg, icase, k1, k2, k3, i1, i2, i3, ii, nd, ndi, indx,
     &        np, flag, ngage, nt2, endx  !, units, limit

      dimension t(5000), z(5000), ti(2600000), zi(2600000), x(26000), 
     &     y(26000), a(26000), b(26000), d(26000), nd(26000), si(26000),
     &     indx(26000), endx(26000), kr(26000)

      character rlab*1, gageid*10, msg*20

      double precision x0, y0, x, y, a, b, d, a12, b12, vmag, proj
      double precision denom1, denom2, test, res12, res23, res13
      double precision c1, c2, c3

      save nrg, ti, zi, nd, x, y, a, b, d, kr, indx, endx, si
...
!              call reportw
     ...
     & call errxit(fname(2), ' eof encountered in rainfile')
        ...
        if (i .eq. 2 .and. .not. xy .and. .not. gage_rain)
     &  call errxit (msg, 'x or y coordinate missing or invalid')

        msg = 'rain gage '//gageid
C                                                                       x coord
        call getr8 ('X', 0, x(i), ierr)

        if (ierr .gt. 0 .and. .not. gage_rain) then
...
C                                                                      get data
     ...
     &    (msg, 'time not increasing')
          ...
          if (k .gt. 2600000) stop ' error - too much rainfall data'
...
C TEMP SNOW
            zi(k) = 0.
C            call errxit (msg,'depth/intensity value missing or invalid')
C TEMP SNOW
          end if
...
C TEMP SNOW
C        if (ti(n+np) .lt. tfins) then
C                                                               add one more pt
C          np = np + 1
C          k = n + np
C          if (k .gt. 2600000) stop ' error - too much rainfall data'
C          ti(k) = tfins + 1.
C          zi(k) = zi(k-1)
...
C   Check for colinearity:
            ...
            denom1 = x(k1) * y(k2) - x(k2) * y(k1)
            denom2 = y(k1) * x(k2) - y(k2) * x(k1)

            if (abs (denom1) .gt. 0. .and. abs (denom2) .gt. 0.) then
              test = (y(k1) - y(k2)) / denom1 * x(k3) +
     &               (x(k1) - x(k2)) / denom2 * y(k3) + 1.
            else
              test = 0.
            end if
...
1111    if (tt .lt. t(j)) then
          ...
          if (j .gt. ndi) then
            ...
            if (ndi + nd(kk) - k .gt. 5000) go to 40
