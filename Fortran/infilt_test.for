...
C
C     ichn         int         1 for planes and main channels, 2 for call
...
C------------------------------------------------------------------------------
      ...
      subroutine infilt ( afac, rfi, h1, qu, dtt, vav, ichn )
...
C------------------------------------------------------------------------------
C TEMP SNOW
      save sk1_snow, por1_snow, smax, fixed_sat, sat_fixed
C TEMP SNOW
...
C!!
C            if(diag .and. j .eq. jd) write(99,877) i, trp, ref*po,
C     &  fap*po, vmin*po, ziu(j), zb(j,i), fr(j,i), rhc !, rc(i), zb(j,i)
C 877  format(/i1,'  time        ref         fpot       vmin        ziu    
C     &       zb          fr          rhc'/9g12.4)
C
C   + + + + + + + + + +  +  +  +  +  +  +   +   +   +   +    +    +    +
            if (ref .lt. vmin ) then
...
C!!  new trace variable
                write(calldat,'("r",i2,"/",i2,i4)')j,nk,itm
...
C                thiur = thur  ! temporary
                f2t = fi(j,i)  
C!!
                write(calldat,'("p",i2"/",I2,i4)')j,nk,itm
...
C!! new trace parameter for rectr call  18/12/02
                write(calldat,'("b",i2,"/",i2,i4)')j,nk,itm
C                                                               estimate rising
...
C                                                             Ks of upper layer
C                                                              (mm/hr or in/hr)
      call getr8 ('KE', 1, sk1_snow, ierr)

      if (ierr .gt. 0) then
        call getr8 ('KS', 1, sk1_snow, ierr)
        if (ierr .gt. 0) sk1_snow = 0.
      end if

C                                                           impervious surface?
      if (sk1_snow .eq. 0.) then
        rsoil = .false.
C        sumtab(nvi)%twola = .false.
C        sumtab(nvi)%thst = 0.
        return
      end if

      sk1_snow = rmks * sk1_snow / (3600. * conv)          ! meters/sec
...
C                                                                of upper layer
     ...
     &annot be > 1.5')
        ...
        call getr8 ('PO', 1, por1_snow, ierr)
...
C TEMP SNOW
      END IF
      sk1(i) = sk1_snow * (1. - ice)
      por1(i) = por1_snow * (1. - ice)
C      sk1(i) = sk1_snow
C      por1(i) = por1_snow
      ser1 = -1.
       if (fixed_sat) then
...
C          sumtab(nvi)%flcap = zfl(i)*(thfc - thin(i)) * (1.-rock1(i))
C
        end if
        thm1(i) = ths1(i)
...
C
C        sumtab(nvi)%pored = zfl(i)*def1(i)*conv   !zfl(i)*por1(i
C
        vmin2(i) = sk2(i)
...
C!!  new self-adjusting time step version of loop
 ...
 277  format(1x,a10,i3,7g12.4/7g12.4/7g12.4)
C          this = 0.5*(thit+thr)
          exit
        end if
C
...
C!!
        ...
        if(dbg .and. nloop .gt. 100) then         !origin 
          ...
          write(99,'(a10,i3,4f9.4/5g13.4/10x,4g12.4)')  origin, nloop,
...
C        end if
      ...
      if(this .lt. thit .or. this .gt. ths) then
        if(dbg) write (99,'(a10," rect out of bounds ",/i4,7g12.4)') 
...
C
        iunit = 77   ! output file should always be opened
C        inquire (99, opened = op)
C        if(op) then  ! only write if diagnostic activated
C          iunit = 99
C          Write(iunit,"(' Se too small in flams from ',a6,/(5g13.4))") 
C     &        trace,  thw, ths, s, a
C        stop ' neg flams '
C        end if
C!! new default
