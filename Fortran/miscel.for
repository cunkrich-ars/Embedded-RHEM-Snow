...
C   Programmed by: C. Unkrich, revised by R.E. Smith 20.02.2002
...
C     nvals         int           number of values actually stored in block,
...
C------------------------------------------------------------------------------
      ...
      integer :: idir(1000), unconn(1000), list(1000), ndir(1000)

      character(len=2) :: attr, adir(1000), free

      real, save :: areas(1000),  storall(1000000) !, adir(100)

      save  limit, icap, idir, list, adir, ndir
...
C                                                 capacity of storall in blocks
      ...
      if (icap .eq. 0) call errxit ('CLERK', 'storage array too small')
...
C------------------------------------------------------------------------------
      ...
      entry old (id, attr, positn, nvals, ierr)
