...
C      logical courc
      ...
      integer :: fileo, storq, storc, typ, sfile, storr
      integer, dimension(1000) :: unconn
      real,dimension(10) :: sumbal
      dimension storq(nchan), storc(5,nchan), sumwso(5) 
...
C------------------------------------------------------------------------------
      ...
      real pwse, pwse_neg, pwse_pos
      integer i, j, l, m, n, k, file1, file2, ierr, 
     &         flag, norphs, ii, outr, idl, fintgs !, ltyp, lfh 
      integer, save :: outc(5,2), outq(2)
C                                                   8
      integer,dimension(10) :: minl, nx, je = (/7,9,8,8,8,7,7,7,7,8/)
      dimension  qso(5,2), pso(5), label(9), q(2),
     &           floss(2)
C** dp() moved to here
      real,dimension(5),save :: wso, wsj, dp
C
      character(LEN=10),dimension(10) :: fmdat = ', 1x,f10.2'
      character(LEN=7),dimension(10)  :: fmhed = ', 1x,A9'
      Character(LEN=9),dimension(3,10) :: hedstr = '         '
      Character(LEN=15) :: charid, head(3)
      Character(LEN=34) :: fminj = '(a14,t50,f10.2                    '
...
C
     ...
     &          slab3*4, alab*2, label*21, wlab*2, buffer*1000,
...
!      character(LEN=2) :: sedlab
      ...
      character(len=8),dimension(0:8) :: eltype = (/'   Plane',
     &  ' Channel','    Pipe','    Pond','Compound','  Inject',
     &  '   Urban','Overbank','   Adder'/)
...
C
     ...
     &     qsp, tsp, wsi, wse, wss, arlab, dref, wse_neg, wse_pos
C
...
C  area over which rain is accumulated
      areaR = qbal(1) 
      ltov = ltab
      if(ltyp .eq. 2 .or. ltyp .eq. 5 .or. ltyp .eq. 8) then
        areaR = 0.  ! no rain for pipes, injects or adders
...
C            twso = twso + wso(n)
C          end do
C          sumtab(ltab)%sedout = twso * wt
C        end if
      end if
C                                                   channel/pond rating exceeded
      if((ltyp .eq. 1 .or. ltyp .eq. 3) .and. iexceed .gt. 0) then
        call blank1
        write(file1, '(a, " rating exceeded at time ", f12.1)')
     &  idbuff, float(iexceed - 1) * delt / 60.
        call blank1
      end if
...
C
      if(rsoil) then
        write(file1,808) thinu, satin, thwilt, satwl, thfc, satfc
        select case ( infm )
          case ( 1 )
            write(file1,810)
          case ( 2 )
            write(file1,811)
          case ( 3 )
            write(file1,809) alfinf
        end select
	end if

  808 format(t44,'theta   rel.sat.'/
     &'  surface initial water content =         ',f6.4,3x,f6.4/
     &'  estimated wilting point water content = ',f6.4,3x,f6.4/
     &'  estimated field capacity =              ',f6.4,3x,f6.4)
  810 format('  Green-Ampt infiltration model used ')
  811 format('  Smith-Parlange infiltration model used ')
  809 format('  3-parameter soil infiltration: alpha = 'f6.4)
...
C------------------------------------------------------------------------------        
        ...
        pwso = twso * wt
        wmax = amax1 (pwso, pwsi, pwss, abs (pwse_neg), pwse_pos )
C --------- Mariano Hernandez 27Mar2013 ---------------------------------------        
C        wmax_neg = amax1 ( pwso, pwsi, pwss, abs (pwse_neg) )
C        wmax_pos = amax1 ( pwso, pwsi, pwss, abs (pwse_pos) )
C -----------------------------------------------------------------------------        
C for graph scaling:
!        gpwso = pwso
!        sedlab = wlab
...
C                                                         write rainfall volume
      ...
      if(.not. chtyp) then
        ...
        else if (vr .eq. 0.) then

          call fmt10 (vr, v2max, string, j)
...
!      call write1 (buffer(1:m))
      ...
      if (sed) then
C ---------- Mariano Hernandez 27Mar2013 --------------       write deposition
        call fmt10 (abs(pwse_pos), wmax, string, j)
        mp = m+1
        m = mp+30
        buffer(mp:m) = '       Deposited: '//string//' '//wlab
...
!       call write1 (buffer(1:m))
            ...
            if (sed) then
C ---------- Mariano Hernandez 27Mar2013              write suspended sediment
        call fmt10 (abs(pwse_neg), wmax, string, j)
        mp = m+1
        m = mp+28
        buffer(mp:m) = '     Soil Loss: '//string//' '//wlab
...
C ------------- Mariano Hernandez 27Mar2013 -----------------------------------
     ...
     &             sumwse_neg, sumwse_pos )

C       entry swrt ( sumwso, sumwsi, sumwse, sumwss, qpk, tpk, storc )
...
C                                                     don't report last element
      ...
      IF( xchan_n > 0 )THEN
        buffer(1:66) = ' Warning - channel IIIIII exceeded maximum depth
     & by XXX.XX percent'
        DO i = 1, xchan_n
          write (buffer(20:25), '(i6)') xchan_id(i)
          write (buffer(53:58), '(F6.2)') xchan_pct(i)
          call writes (buffer(1:66))
        END DO
        call blanks
      END IF

      IF( xpond_n > 0 )THEN
        buffer(1:56) = ' Warning - pond IIIIII exceeded rating by XXX.XX
     & percent'
        DO i = 1, xpond_n
          write (buffer(17:22), '(i6)') xpond_id(i)
          write (buffer(43:48), '(F6.2)') xpond_pct(i)
          call writes (buffer(1:56))
        END DO
        call blanks
      END IF
...
C    
          ...
          case(1)                    ! element area
            ...
            hedstr(2,1) = ' Area    '
            Hedstr(3,1) = '  '//arlab//'   '
            write(fmdat(1)(10:10),'(I1)') ndp
          case(2)                    !  cum area
            lneed = nma + 1 + ndp
            write(fmdat(2)(10:10),'(I1)') ndp
            hedstr(1,2) = 'Cumulated'
            hedstr(2,2) = '  Area   '
            hedstr(3,2) = '  '//arlab//'   '
          case(3,5,7)             ! inflow, outflow, total infil
            lneed = nmq + 1 + ndq
            write(fmdat(jc)(10:10),'(I1)')  ndq
...
C    
          ...
          lfd = lfd + minl(jc) + 1
C    
          write(fmhed(jc)(2:3),'(I2)') nx(jc)
          write(fmhed(jc)(7:7),'(I1)') je(jc)
          write(fmdat(jc)(7:8),'(I2)') minl(jc)
C    
...
C                     first go thru list and report planes, injects, and urban els.
        do jp = 1,ltab
          itu = sumtab(jp)%itype
          if(itu .eq.0 .or. itu .eq.6 .or. itu .eq.5 .or. itu.eq.8) then
            write(charid(1:7),'(I6,1x)') sumtab(jp)%idel
            charid(8:15) = eltype(itu)
            if(itu .eq. 5) then
              fminj = '(a15,39x,f10.3,1x,f7.3)'
...
C
C TEMP SNOW
C     Write year, month, day, sat, rainfall volume (mm), runoff volume (mm), sediment yield (kg/ha)
C       if(snow) 
C     & write(99,'(A4,",",A5,",",F12.4,",",F12.4,",",F12.4,",",F12.4)')
C     &  yr_str, mmdd_str, satin, sumbal(2)/sumbal(1)*conv,
C     &  sumbal(10)/sumbal(1)*conv, twso
C TEMP SNOW
C
      return
