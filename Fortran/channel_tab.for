...
C------------------------------------------------------------------------------
     ...
     &        leng, upq,  i, inflow, idup, upr, idlat, outr  
      ...
      character msg*20, c*1, string*10, source*6
...
C** new chl
     ...
     &              rf(20,2), qltr(20), afov(20), rwidth(20,2)
      ...
      integer,dimension(2) :: outq, latq, latr, nql
      real,dimension(5000) :: t, z, r
      real,dimension(1000) :: q, v, h
      real,dimension(2) :: dqu, qu, qu1, qu2, z1, z2, z3, qlat1, qlat2,
     &                     dthr, dqlat, zb, quba
C
      dimension  qt(20), topw(20,2), slope(20,2), rs(2,2), inflow(12), 
     &           upq(10,2), qup(10,2), upr(10), ql(20,2), depth(20),
     &           idup(10), idlat(2), afac(20), hr1(20,2), nq(10,2)
     
      real, parameter :: dtmin = 1.0
C                                                                   common data
C------------------------------------------------------------------------------

      COMMON /tab/ tabdat, n1, h_tab(500), a_tab(500,20,2),
     &             wp_tab(500,20,2), tw_tab(500,20,2), j_tab

      LOGICAL tabdat

      REAL da_tab(500), dwp_tab(500), dtw_tab(500)
...
C** 22/4/03  dx check      
 ...
 1990 format(/"  Channel ",i6,":  based on length and parameter CLEN,"/
 ...
 !     
     ...
     &1x,a2," each.")') nkm, dx, dlab
      ...
      CALL geti4 ('N1', 1, n1, ierr) ! upstream tabular data

      IF( ierr .EQ. 0 )THEN
        CALL getr4 ('DZ', 0, dz, ierr)
        IF (ierr .gt. 0)
     &  CALL errxit (msg(ms:18),'elevation increment (DZ) not found')
        tabdat = .TRUE.
        chrain = .FALSE.
        DO i = 1, n1
          h_tab(i) = REAL( i - 1 ) * dz
        ...
        DO i = n1, 1, -1
          k = n1 - i + 1
          CALL getr4 ('A1', k, a_tab(i,1,1), ierr)
          IF (ierr .gt. 0)
     &    CALL errxit (msg(ms:18),'missing area (A1)')
          CALL getr4 ('WP1', k, wp_tab(i,1,1), ierr)
          IF (ierr .gt. 0)
     &    CALL errxit (msg(ms:18),'missing wetted perimeter (WP1)')
          CALL getr4 ('TW1', k, tw_tab(i,1,1), ierr)
          IF (ierr .gt. 0)
     &    CALL errxit (msg(ms:18),'missing top width (TW1)')
        END DO

        IF( a_tab(1,1,1) > 0. )THEN ! insert zero area at h = 0
          a_tab(2:n1+1,1,1)  = a_tab(1:n1,1,1)
          wp_tab(2:n1+1,1,1) = wp_tab(1:n1,1,1)
          tw_tab(2:n1+1,1,1) = tw_tab(1:n1,1,1)
          a_tab(1,1,1)  = 0.
          wp_tab(1,1,1) = .01
          tw_tab(1,1,1) = 0.
          h_tab(n1+1)   = REAL( n1 ) * dz
          n1 = n1 + 1
        ELSE  IF (wp_tab(1,1,1) .eq. 0.) THEN ! wetted perimeter cannot be zero (denominator in q eq.)
          wp_tab(1,1,1) = .01
        END IF

        CALL geti4 ('N2', 1, n2, ierr) ! downstream tabular data

        IF( ierr .EQ. 0 )THEN
          DO i = n2, 1, -1
            k = n2 - i + 1
            CALL getr4 ('A2', k, a_tab(i,nk,1), ierr)
            IF (ierr .gt. 0)
     &      CALL errxit (msg(ms:18),'missing area (A2)')
            CALL getr4 ('WP2', k, wp_tab(i,nk,1), ierr)
            IF (ierr .gt. 0)
     &      CALL errxit (msg(ms:18),'missing wetted perimeter (WP2)')
            CALL getr4 ('TW2', k, tw_tab(i,nk,1), ierr)
            IF (ierr .gt. 0)
     &      CALL errxit (msg(ms:18),'missing top width (TW2)')
          END DO

          IF( a_tab(1,nk,1) > 0. )THEN ! insert zero area at h = 0
            a_tab(2:n2+1,nk,1)  = a_tab(1:n2,nk,1)
            wp_tab(2:n2+1,nk,1) = wp_tab(1:n2,nk,1)
            tw_tab(2:n2+1,nk,1) = tw_tab(1:n2,nk,1)
            a_tab(1,nk,1)  = 0.
            wp_tab(1,nk,1) = .01
            tw_tab(1,nk,1) = 0.
            h_tab(n2+1)    = REAL( n2 ) * dz
            n2 = n2 + 1
          ELSE IF (wp_tab(1,nk,1) .eq. 0.) THEN ! wetted perimeter cannot be zero (denominator in q eq.)
            wp_tab(1,nk,1) = .01
          END IF

          IF( n2 > n1 )THEN ! extrapolate upstream parameters
            da  = a_tab(n1,1,1)  - a_tab(n1-1,1,1)
            dwp = wp_tab(n1,1,1) - wp_tab(n1-1,1,1)
            dtw = tw_tab(n1,1,1) - tw_tab(n1-1,1,1)
            DO i = n1 + 1, n2
              h_tab(i) = REAL( i - 1 ) * dz
              a_tab(i,1,1)  = a_tab(i-1,1,1)  + da
              wp_tab(i,1,1) = wp_tab(i-1,1,1) + dwp
              tw_tab(i,1,1) = tw_tab(i-1,1,1) + dtw
            END DO
            n1 = n2
          ELSE IF( n1 > n2 )THEN ! extrapolate downstream parameters
            da  = a_tab(n2,nk,1)  - a_tab(n2-1,nk,1)
            dwp = wp_tab(n2,nk,1) - wp_tab(n2-1,nk,1)
            dtw = tw_tab(n2,nk,1) - tw_tab(n2-1,nk,1)
            DO i = n2 + 1, n1
              h_tab(i) = REAL( i - 1 ) * dz
              a_tab(i,nk,1)  = a_tab(i-1,nk,1)  + da
              wp_tab(i,nk,1) = wp_tab(i-1,nk,1) + dwp
              tw_tab(i,nk,1) = tw_tab(i-1,nk,1) + dtw
            END DO
          END IF
        ELSE
          DO i = 1, n1
            a_tab(i,nk,1)  = a_tab(i,1,1)
            wp_tab(i,nk,1) = wp_tab(i,1,1)
            tw_tab(i,nk,1) = tw_tab(i,1,1)
          END DO
        END IF
      ELSE
        tabdat = .FALSE.
...
!      WRITE( 88, '(4X,200I12)' ) (k, k = 1, n1)
!      WRITE( 88, '(I4,200F12.4)' )  1, (a_tab(1,1,1), k = 1, n1)
!      WRITE( 88, '(I4,200F12.4)' )  1, (wp_tab(1,1,1), k = 1, n1)
!      WRITE( 88, '(I4,200F12.4)' ) nk, (a_tab(k,nk,1), k = 1, n1)
!      WRITE( 88, '(I4,200F12.4)' ) nk, (wp_tab(k,nk,1), k = 1, n1)
...
C                                                                       SLOPE
     ...
     &      (msg(ms:18), 'main section slope (SL) not found')
          ...
          if (slope(j,1) .le. 0.) call errxit (msg(ms:18), 'zero slope')
        end if
C                                                                       MANNING?
...
!        dco1 = (co1(nk,1) - co1(1,1)) / rnk
        ...
        if(chrain) then
          ...
          rwidth(nk,1) = width(nk,1) + depth(nk)*co1(nk,1)
      ...
      IF( tabdat )THEN
        DO i = 1, n1
          da_tab(i)  = (a_tab(i,nk,1)  - a_tab(i,1,1))  / rnk
          dwp_tab(i) = (wp_tab(i,nk,1) - wp_tab(i,1,1)) / rnk
          dtw_tab(i) = (tw_tab(i,nk,1) - tw_tab(i,1,1)) / rnk
        END DO
      END IF
...
!         co1(j,1) = co1(1,1) + dco1 * rj
        ...
        IF( tabdat )THEN
          ...
          DO i = 1, n1
            a_tab(i,j,1)  = a_tab(i,1,1)  + da_tab(i)  * rj
            wp_tab(i,j,1) = wp_tab(i,1,1) + dwp_tab(i) * rj
            tw_tab(i,j,1) = tw_tab(i,1,1) + dtw_tab(i) * rj
          END DO

C         Compute spline coefficients at each depth

!          CALL spline( h_tab,  a_tab(j,1,:), n1, 1.0E30, 1.0E30,
!     &                 a_tab(j,2,:) )
!          CALL spline( h_tab, wp_tab(j,1,:), n1, 1.0E30, 1.0E30,
!     &                 wp_tab(j,2,:) )
!          CALL spline( h_tab, tw_tab(j,1,:), n1, 1.0E30, 1.0E30,
!     &                 tw_tab(j,2,:) )
      ...
      IF( tabdat )THEN ! Compute spline derivatives
!        WRITE( 88, '(4X,200I12)' ) (k, k = 1, n1)
        DO j = 1, nk
          CALL spline_pchip_set( n1,h_tab, a_tab(:,j,1), a_tab(:,j,2) )
          CALL spline_pchip_set( n1,h_tab,wp_tab(:,j,1),wp_tab(:,j,2) )
          CALL spline_pchip_set( n1,h_tab,tw_tab(:,j,1),tw_tab(:,j,2) )
!          WRITE( 88, '(I4,200F12.4)' ) j, (a_tab(k,j,2), k = 1, n1)
!          WRITE( 88, '(I4,200F12.4)' ) j, (wp_tab(k,j,2), k = 1, n1)
        END DO
...
C------------------------------------------------------------------------------
      ...
      do j = 1, nk
        ...
        IF( .NOT. tabdat )THEN
          ...
          ewpr1(j,1) = width(j,1)
        ELSE
          wpr2(j,1)  = wp_tab(1,j,1)
          wpr1(j,1)  = wp_tab(1,j,1)
          ewpr1(j,1) = wpr1(j,1)
        END IF        
...
C      bedvol = bedvol + barea * 0.5 * slope(1,1)**-1.4
...
C                                                               ...and sediment
      if (sed) call sed0 ( msg(ms:18), inflow, nu, nm,
...
C          qbal(2) = qbal(2) + xarea*z(nd)
          ...
          qbal(1) = 0. ! qbal(1) + xleng * 0.5*(width(1,2) + width(nk,2))
        end if
...
C                                                               ...and sediment
        if (sed) call sed0 ( msg(ms:18), inflow, nu, nl,
     &                       nk, omega, dx, slope, vone)
...
C------------------------------------------------------------------------------
      ...
      do k = 1, nu
        ...
        call old (idup(k), 'RA', upr(k), nr, ierr)

        call old (idup(k), 'RO', upq(k,1), nq(k,1), ierr)

        if (nq(k,1) .gt. nq_max) nq_max = nq(k,1)
...
C moved        inflow(k) = idup(k)
        ...
        call old (idup(k), 'ro', upq(k,2), nq(k,2), ierr)

        if (nq(k,2) .gt. nq_max) nq_max = nq(k,2)
      ...
      do k = 1, nl
        ...
        call old (idlat(k), 'RA', latr(k), nr, ierr)

        call old (idlat(k), 'RO', latq(k), nql(k), ierr)

        if (nql(k) .gt. nq_max) nq_max = nql(k)
...
C*2/3/2004 index j needs to be set to one (see changes to errhub)
	    j = 1
          ...
          call iter (errhub, h1(1,1), 0., h_lim, ierr, source)
     ...
     &      (msg(ms:18),  'incoming baseflow > threshold')
          ...
          if (h1(1,1) .gt. hmax) hmax = h1(1,1)

          IF( tabdat )THEN
            if (h1(1,1) .gt. h_tab(n1)) iexceed = 1
          ELSE
            if (h1(1,1) .gt. depth(1)) iexceed = 1
          END IF
...
C                                                                    flow depth
        ...
        IF( sed )THEN
          IF( tabdat )THEN
!            CALL splint( h_tab, tw_tab(1,1,:), tw_tab(1,2,:),
!     &      n1, h1(1,1), topw(1,1), dtopw, ierr )

             CALL spline_pchip_val( n1, h_tab, tw_tab(:,1,1),
     &                              tw_tab(:,1,2), h1(1,1),
     &                              topw(1,1), dtopw )
          ELSE
            topw(1,1) = width(1,1) + h1(1,1) * co1(1,1)
          END IF
        END IF
        ...
        do j = 2, nk
          ...
          if (qub .gt. 0.) then
            ...
            call iter (errhub, h1(j,1), 0., h_lim, ierr, source)
     ...
     &      (msg(ms:18),  'baseflow > threshold')
            ...
            if (h1(j,1) .gt. hmax) hmax = h1(j,1)

            IF( tabdat )THEN
              if (h1(j,1) .gt. h_tab(n1)) iexceed = 1
            ELSE
              if (h1(j,1) .gt. depth(j)) iexceed = 1
            END IF
            ...
            IF( sed )THEN
              IF( tabdat )THEN
!                CALL splint( h_tab, tw_tab(j,1,:), tw_tab(j,2,:),
!     &          n1, h1(j,1), topw(j,1), dtopw, ierr )

                 CALL spline_pchip_val( n1, h_tab, tw_tab(:,j,1),
     &                                  tw_tab(:,j,2), h1(j,1),
     &                                  topw(j,1), dtopw )
              ELSE
                topw(j,1) = width(j,1) + h1(j,1) * co1(j,1)
              END IF
            END IF
...
C                                                 compute initial concentration
        if (sed) call sedbf (qup, a1, q1, topw)
...
C Minimum simulation time
      if (chrain) then
        t_min = max (t(nd), float (nq_max - 1) * delt)
      else
        t_min = float (nq_max - 1) * delt
      end if

C TEMP YUVAL
C      WRITE( 88,'(I6)' ) id
C      WRITE( 88,'(I6,",",F16.4)' ) 0, 0.
C END TEMP
...
C------------------------------------------------------------------------------
            source = 'ChnUB2'
            if(h2(1) .le. 0.) then
              h2(1) = (qub/wpr1(1,1)/alpha(1,1))**(1./p) ! init. estimate
            end if
C*2/3/2004 index j needs to be set to one (see changes to errhub)
	      j = 1
            j_tab = 1

            call iter (errhub, h2(1), 0., h_lim, ierr, source)
            if(h2(1) .lt. 0.) h2(1) = 0.
            ...
            if (h2(1) .gt. hmax) hmax = h2(1)

            IF( tabdat )THEN
              if (h2(1) .gt. h_tab(n1) .and. iexceed .eq. 0) iexceed = i
            ELSE
              if (h2(1) .gt. depth(1) .and. iexceed .eq. 0) iexceed = i
            END IF
...
C     &                                      wpr2(1,2), wco)  ! new 10/01
     ...
     &                           a2(1,2), hh, dadh2, co2hb(1))
              else

                hhh = hh - hb(1)
                dadh2 = dadhf (twhb(1), hhh, co1(1,2))
                dqdh2 = dqdhf (alpha(1,2), wpr2(1,2), p,
     &                           a2(1,2), hhh, dadh2, co2(1,2))
...
C------------------------------------------------------------------------------
            ...
            call iter (errha, h2a, 0., h_lim, ierr, source)
            if(h2a .lt. 0.) h2a = 0.

            if (h2a .gt. hmax) hmax = h2a

            IF( tabdat )THEN
              if (h2a .gt. h_tab(n1) .and. iexceed .eq. 0) iexceed = i
            ELSE
              if (h2a .gt. depth(2) .and. iexceed .eq. 0) iexceed = i
            END IF

            h2(2) = h2a
            dadh = dadhf (width(2,1), h2a, co1(2,1))
            dqdh = dqdhf (alpha(2,1), wpr2(2,1), p,
     &                    a2(2,1), h2a, dadh, co2(2,1))
C                                                              compute celerity
            vc = dqdh / dadh
            xc = xc + dt * vc
            if (vc .gt. vcm) vcm = vc
...
C                                                       begin kinematic routing
              ...
              do j = 3, nk
                ...
                IF( tabdat )THEN
                  if (h2a .gt. h_tab(n1) .and.iexceed.eq. 0) iexceed = i
                ELSE
                  if (h2a .gt. depth(j) .and. iexceed .eq.0) iexceed = i
                END IF
...
C  route flow for general case
            ...
            call iter (errch2, h2jp1, 0., h_lim, ierr, source)
 ...
 177  format(" J    h1(j,1)      h2(j,1)       h1(jp1,1)      h2(jp)    
     &   fav(jp1)"/I2,6g13.4)
            ...
            if (h2jp1 .gt. hmax) hmax = h2jp1

            IF( tabdat )THEN
              if (h2jp1 .gt. h_tab(n1) .and. iexceed .eq. 0) iexceed = i
            ELSE
              if (h2jp1 .gt. depth(jp1) .and. iexceed .eq.0) iexceed = i
            END IF
     ...
     &                         a2(jp1,1), h2jp1, dadh1, co2(jp1,1))
              dadh2 = 0.
              dqdh2 = 0.
...
C*TEMP YUVAL
C      WRITE( 88,'(I6)' ) id
C      WRITE( 88,'(I6,",",F16.4)' ) int(time/60.), h2(nk)
C*END TEMP

C*TEMP
!      WRITE( 88, '(I6,2F16.4)') i, (qlat(j), j = 1, 2)
!      WRITE( 88, '(I6,15F16.4)') i, (h2(j), j = 1, nk)
!      WRITE( 88, '(I6,15F16.4)') i, (a2(j,1), j = 1, nk)
!      WRITE( 88, '(I6,15F16.4)') i, (wpr2(j,1), j = 1, nk)
!      WRITE( 88, '(I6,15F16.4)') i, (ewpr2(j,1), j = 1, nk)
!      WRITE( 88, '(I6,15F16.4)') i, (fav(j,1)*1000.*60.*60., j = 1, nk)
!      WRITE( 88, '(" ")')
C*TEMP
  
C 
C After rainfall and all inflows cease, stop simulating when no water
C remains on the surface

        if(time .gt. t_min .and. qtest .eq. 0.) go to 100
...
C*2/2/2004 subtract initial baseflow profile
      ...
      IF( tabdat )THEN
        IF( hmax > h_tab(n1) )THEN
          xchan_n = xchan_n + 1
          xchan_id(xchan_n) = id
          xchan_pct(xchan_n) = (hmax - h_tab(n1)) / h_tab(n1) * 100.
        END IF
      END IF
C                                                         pass values to writer
C------------------------------------------------------------------------------

      call qwrt (id,nchan,msg,qp,tp,vin,coarea,outq,ltyp,outr,qrmax)
...
! New declaration
     ...
     &                    a2(jp1,1), h2jp1, da2adh, co2(jp1,1))
        da2bdh = 0.
        dq2bdh = 0.
...
!          ewpr2(jp1) = wpr2(jp1,1)
     ...
     &             p, a2(jp1,2), hh, da2bdh, co2hb(jp1))
     ...
     &                         co1hb(jp1), wco)
          ...
          dq2bdh = dqdhf (alpha(jp1,2), wpr2(jp1,2),
     &             p, a2(jp1,2), hhh, da2bdh, co2(jp1,2))
...
C------------------------------------------------------------------------------
     ...
     &             wp_tab(500,20,2), tw_tab(500,20,2), j_tab
      ...
      IF( tabdat )THEN
        ...
        IF( h > 0. )THEN
          CALL spline_pchip_val( n1, h_tab, a_tab(:,j_tab,1),
     &                           a_tab(:,j_tab,2), h, a, da )
          areaf = a
        ELSE
          areaf = 0.
        END IF
     ...
     &             wp_tab(500,20,2), tw_tab(500,20,2), j_tab
      ...
      IF( tabdat )THEN
        ...
        IF( h > 0. )THEN
          CALL spline_pchip_val( n1, h_tab, wp_tab(:,j_tab,1),
     &                           wp_tab(:,j_tab,2), h, wpr, dwpr )
          cwprf = wpr
        ELSE
          cwprf = wp_tab(1,j_tab,1)
        END IF
     ...
     &                           a_tab(:,j_tab,2), h, a, da )
      ...
      function dqdhf (al, wp, p, ar, h, dadh, co2)
     ...
     &             wp_tab(500,20,2), tw_tab(500,20,2), j_tab
      ...
      IF( tabdat )THEN
        ...
        IF( h > 0. )THEN
          CALL spline_pchip_val( n1, h_tab, wp_tab(:,j_tab,1),
     &                           wp_tab(:,j_tab,2), h, wpr, dwpdh )
        ELSE
          dwpdh = wp_tab(1,j_tab,2)
        END IF
      ELSE
