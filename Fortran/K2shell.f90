...
! TEMP SNOW
  ...
  CHARACTER(LEN=4) :: yr_str1, yr_str2
  INTEGER, PARAMETER :: DAYS(12) = (/0,31,59,90,120,151,181,212,243,273,304,334/)
  first_event = .TRUE.
  q_sum = 0.
  s_sum = 0.
  q_tot = 0.
...
! TEMP SNOW
      j = INDEX( fields(2), '.' )
      yr_str2  = fields(2)(j-10:j-7)
      mmdd_str = fields(2)(j-5:j-4)//','//fields(2)(j-2:j-1)
!      yr_str2  = fields(2)(17:20)
!      READ( fields(2)(22:24), * ) k ! doy
!      DO j = 12, 1, -1
!        IF( DAYS(j) < k ) EXIT
!      END DO
...
!      WRITE( mmdd_str, '(I2.2,",",I2.2)' ) j, i
      IF( first_event )THEN
        yr_str1  = yr_str2
        OPEN( 98, FILE = fields(3), STATUS = 'UNKNOWN' )
        j = INDEX( fields(3), '.' )
        OPEN( 99, FILE = fields(3)(1:j-1)//'_events.csv', STATUS = 'UNKNOWN' )
...
!   Write year, month, day, sat, rainfall volume (mm), runoff volume (mm), sediment yield (kg/ha)
    ...
    IF( q_snow > 0. )THEN
      WRITE(99,'(A4,",",A5,",",F12.4,",",F12.4,",",F12.4,",",F12.4)') yr_str2, mmdd_str, satin, qbal(2)/qbal(1)*conv, q_snow, s_snow
    END IF
    IF( yr_str2 == yr_str1 )THEN
      q_sum = q_sum + q_snow
      s_sum = s_sum + s_snow
    ELSE
      WRITE( 98, '(A4,",",F6.1,",",F8.3)') yr_str1, q_sum, s_sum/1000. ! mm, Mg/ha
      q_tot = q_tot + q_sum
      s_tot = s_tot + s_sum
      q_sum = q_snow
      s_sum = s_snow
...
! TEMP SNOW
  ...
  write (*, "(' Elapsed time (min) ', i3.3)") te / 60
